...
class Inx_Apiimpl_SplitTest_SplitTestManagerImpl implements Inx_Api_SplitTest_SplitTestManager, Inx_Apiimpl_Core_BOResultSetDelegate
{
    /**
    ...
    public function get($iId)
    {
        ...
        try
        {
            ...
            if (empty($data))
                throw new Inx_Api_DataException("No split test found for ID: " . $iId);

            return new Inx_Apiimpl_SplitTest_SplitTestImpl($this->_oSession, $data);
        }
     ...
     * @return array of Inx_Apiimpl_SplitTest_SplitTestManagerImpl's
     * @throws Inx_Api_RemoteException
     */
    public function fetchBOs(Inx_Apiimpl_RemoteRef $oResultSetRef, $iIndex, $iDirection)
    {
        return Inx_Apiimpl_SplitTest_SplitTestImpl::convert($this->_oSession,
